...
TS_TOKENS = {
    "eos_token": "<ts>",
    "pad_token": "<|endoftext|>",
    "additional_special_tokens": ["<speaker1>", "<speaker2>", "<ds>", "</ds>", "/a", "/b"],
}
...
class SpokenDialogTokenizer(SpokenNormalizer):
    ...
    def __init__(
        ...
        if self.language == "Japanese":
            exist_backchannel = ['なんか', 'なんかー', 'なんかね', 'なんかねー', 'あの', 'あのー', 'あのね', 'その', 'そのー', 'でー',  
            ...
            interjection = list(set(ACK_BACK['ja'] + AGREE_ACCEPT['ja'] + NEGATIVE_FEEDBACK['ja'] + exist_backchannel))
        if self.language == "English":
            exist_backchannel = ['alright','ehm','ha--','ah','aye','eh', 'nah','erm',"'kay",'yay','a--','okey-dokey', 'mm-mm','uh-uh',
                                'well--','phoar','yee-ha','r--', 'rightee-ho','surely','aha','pardon','un--', 'yup','m--','y--','oo','so']
            interjection = list(set(ACK_BACK['en'] + AGREE_ACCEPT['en'] + ANSWERS['en'] + NEGATIVE_FEEDBACK['en'] + exist_backchannel))

        new_tokens = [word for word in interjection if len(word.split()) == 1 and word not in self._tokenizer.get_vocab()]
    ...
    def idx_to_tokens(self, ids):
        def list_ids_to_string(ids):
            return [
                str(t) for t in self.convert_ids_to_tokens(ids)
            ]
            #return [
