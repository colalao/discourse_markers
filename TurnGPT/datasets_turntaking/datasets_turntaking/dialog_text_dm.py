...
class ConversationalDM(pl.LightningDataModule):
    ...
    def __init__(
        ...
        self.savepath = join(savepath, self.DATASETS[0])
        self.overwrite = overwrite

    @classmethod
    def set_datasets(cls, language):
        if language == "Japanese":
            cls.DATASETS = ["Japanese"]
        elif language == "English":
            cls.DATASETS = ["English"]
        else:
            raise ValueError(f"Invalid test_type: {language}. Allowed values are: 'Japanese', 'English'.")
...
if __name__ == "__main__":
    ...
    ConversationalDM.set_datasets("English")  # set language to English for testing
